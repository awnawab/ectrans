...
CONTAINS
  ...
  SUBROUTINE TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    LOGICAL :: LPGPUV_PRESENT = .FALSE.
    LOGICAL :: LPGP3A_PRESENT = .FALSE.
    LOGICAL :: LPGP2_PRESENT = .FALSE.
...
#define TRGTOL_DTYPE MPI_REAL8
...
#ifdef ACCGPU
    IF (PRESENT(PGPUV)) LPGPUV_PRESENT = ACC_IS_PRESENT(PGPUV)
    IF (PRESENT(PGP3A)) LPGP3A_PRESENT = ACC_IS_PRESENT(PGP3A)
    IF (PRESENT(PGP2)) LPGP2_PRESENT = ACC_IS_PRESENT(PGP2)
#endif
    ...
    ENDIF
    IF (PRESENT(PGPUV) .AND. .NOT. LPGPUV_PRESENT) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGPUV)
    ENDIF
    IF (PRESENT(PGP2) .AND. .NOT. LPGP2_PRESENT) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
      ACC_POINTERS(ACC_POINTERS_CNT) = EXT_ACC_PASS(PGP2)
    ENDIF
    IF (PRESENT(PGP3A) .AND. .NOT. LPGP3A_PRESENT) THEN
      ACC_POINTERS_CNT = ACC_POINTERS_CNT + 1
...
#endif
#ifdef ACCGPU
        !$ACC UPDATE DEVICE(PGPUV) IF (.NOT. LPGPUV_PRESENT)
#endif
    ENDIF
...
#endif
#ifdef ACCGPU
        !$ACC UPDATE DEVICE(PGP2) IF (.NOT. LPGP2_PRESENT)
#endif
    ENDIF
...
#endif
#ifdef ACCGPU
        !$ACC UPDATE DEVICE(PGP3A) IF (.NOT. LPGP3A_PRESENT)
#endif
    ENDIF
    IF (PRESENT(PGP3B)) THEN
#ifdef OMPGPU
